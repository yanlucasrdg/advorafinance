...
const navGroups: NavGroup[] = [
  ...
  {
    title: "Administração",
    items: [{ to: "/admin/usuarios", label: "Usuários", icon: ShieldCheck }],
  },
];
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    BRAND_CSS_VARIABLE_NAMES.forEach((name) => root.style.removeProperty(name));
    if (!branding) {
      return;
    }

    const variables = getBrandCssVariables(branding.primary_color, branding.secondary_color);
  ...
  const crumbs = useMemo(() => {
    ...
    return [
      { label: branding?.brand_name ?? "Advora", to: "/dashboard" },
      { label, to: root },
    ];
            ...
            <div className="space-y-0.5">
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <item.icon
                      className={`size-[16px] shrink-0 ${active ? "text-primary" : "text-muted-foreground group-hover:text-foreground"}`}
    ...
    <div className="min-h-screen flex bg-background">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-[72px] border-b border-border bg-background/85 backdrop-blur-xl flex items-center gap-3 px-4 sm:px-6 sticky top-0 z-30">
          ...
          {/* Search (centered) */}
          <div className="flex items-center gap-2.5 max-w-[480px] flex-1 min-w-0 mx-auto h-10 px-3.5 rounded-xl border border-border bg-card hover:border-foreground/15 focus-within:border-ring/50 focus-within:shadow-[0_0_0_3px_var(--brand-focus-ring)]">
          ...
          <div className="flex items-center gap-1 ml-auto shrink-0">
            ...
            <Link
              to="/copiloto"
              className="inline-flex items-center gap-2 h-9 px-3.5 rounded-lg bg-primary text-[13px] font-semibold text-primary-foreground shadow-[var(--shadow-sm)] hover:bg-[var(--primary-hover)] hover:shadow-[var(--shadow-md)]"
            >
